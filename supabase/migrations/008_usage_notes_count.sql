-- Add monthly cumulative notes counter to usage
alter table if exists usage
  add column if not exists notes_count int not null default 0;

-- Remove legacy overload to avoid PostgREST RPC ambiguity
...
-- Backfill monthly note creations from history for existing projects
insert into usage (user_id, month, notes_count)
select
  h.user_id,
  date_trunc('month', h.created_at)::date as month,
  count(*)::int as notes_count
from history h
where h.module = 'note'
group by h.user_id, date_trunc('month', h.created_at)::date
on conflict (user_id, month) do update set
  notes_count = greatest(usage.notes_count, excluded.notes_count);

-- Extend atomic usage increment function to support notes_count
